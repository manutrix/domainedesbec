...
foreach($selectSQL as $value){
    ...
    echo '<td align="center" class="cg_button_edit"><p><a href="?page='.cg_get_version().'/index.php&option_id='.$option_id.'
    ...
    echo '<td align="center" class="cg_button_copy"><p><form action="?page='.cg_get_version().'/index.php&edit_gallery=true" method="POST" class="cg_load_backend_copy_gallery" >';
    ...
    echo '<input type="hidden" name="page" value="'.cg_get_version().'/index.php"><input name="" value="Copy" type="Submit" id="cgCopySubmit'.$option_id.'" class="'.$cg_copy_submit.'" data-cg-version-to-copy="'.$Version.'"
    data-cg-copy-fb-on="'.$FbLike.'" data-cg-copy-id="'.$option_id.'"></form></p></td>';

    // DELETE GALLERY

    echo '<td align="center" class="cg_button_delete"><p><form action="?page='.cg_get_version().'/index.php" method="GET"  class="cg_load_backend_submit" >
...
echo "<table style='border: thin solid black;background-color:#ffffff;' width='937px'>";
echo '<tr><td style="padding-left:20px;overflow:hidden;" colspan="4"><p><form action="?page='.cg_get_version().'/index.php&option_id='.$nextID.'&edit_gallery=true" method="POST" class="cg_load_backend_submit cg_load_backend_create_gallery" >';
echo '<input type="hidden" name="cg_create" value="true">';
echo '<input type="hidden" name="option_id" value="'.$nextID.'">';
echo '<input type="hidden" name="create" value="true"><input type="hidden" name="page" value="'.cg_get_version().'/index.php">
