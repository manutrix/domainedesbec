...
// Allow only to press numbers as keys in input boxes
    ...
    $(document).on('keypress',"#ScaleSizesGalery1, #ScaleSizesGalery2, #DistancePicsV, #DistancePicsV, #PicsInRow, #PicsPerSite,#ThumbViewBorderRadius,#RowViewBorderRadius,#HeightViewBorderRadius,#HeightViewSpaceHeight,#WidthThumb,"+
        "#PostMaxMB, #VotesPerUser, #VotesPerCategory, #RegUserMaxUpload, #VotesInTimeQuantity, #BulkUploadQuantity,#BulkUploadMinQuantity, #DistancePics, #MaxResJPGwidth, #MaxResJPGheight, #MaxResPNGwidth, #MaxResPNGheight, #MaxResGIFwidth, #MaxResGIFheight, #cg_row_look_border_width,#cg_height_look_border_width,#HeightViewBorderWidth,.HeightLookHeight",function (e) {
...
//JPG
    ...
    $(document).on('click',"#MaxResJPGonContainer",function(e){
        //cgViewOptionCheck(this,e);
...
//PNG
    ...
    $(document).on('click',"#MaxResPNGonContainer",function(e){
        //cgViewOptionCheck(this,e);
...
//GIF
    ...
    $(document).on('click',"#MaxResGIFonContainer",function(e){
        //cgViewOptionCheck(this,e);
...
/*    var cgRegUserUploadOnly = function(){
    ...
    $(document).on('click',".CheckMethodUploadContainer",function(e){
        //cgViewOptionCheck(this,e);
        cgJsClassAdmin.options.functions.cg_user_reocgnising_method_upload($,$(this).find('.CheckMethodUpload'));
    ...
    $( window ).scroll(function() {
        ...
        $cg_view_select_objects.each(function () {
            ...
            var $viewHeader = $cg_main_options_content.find($cg_view_select.attr('data-view'));
                    ...
                    if($cg_main_options_tab.hasClass('cg_sticky')){
                        if(viewCount==1){
                            if(!cg_short_code_multiple_pics_configuration_cg_sticky_removed){
                                $cg_main_options.find('.cg_short_code_multiple_pics_configuration_buttons').addClass('cg_sticky');
                            }
                            $cg_main_options.find('.cg_short_code_single_pic_configuration_buttons').removeClass('cg_sticky');
                        }else if(viewCount==2){
                            $cg_main_options.find('.cg_short_code_multiple_pics_configuration_buttons').removeClass('cg_sticky');
                            if(!cg_short_code_single_pic_configuration_buttons_cg_sticky_removed){
                                $cg_main_options.find('.cg_short_code_single_pic_configuration_buttons').addClass('cg_sticky');
                            }
                        }else{
                            $cg_main_options.find('.cg_short_code_multiple_pics_configuration_buttons').removeClass('cg_sticky');
                            $cg_main_options.find('.cg_short_code_single_pic_configuration_buttons').removeClass('cg_sticky');
    ...
    $(document).on('click','a[href="#cgGoToImageViewBackgroundStyle"]',function (e) {
        e.preventDefault();
        //cgViewOptionCheck(this,e);
        var $cg_image_view_background_style= $('.cg_short_code_single_pic_configuration_container').not('.cg_hide').first().find('.GalleryStyleContainer');
        $(window).scrollTop($cg_image_view_background_style.offset().top-400);
        $cg_image_view_background_style.addClass('cg_blink');
        setTimeout(function (){
            $cg_image_view_background_style.removeClass('cg_blink');
        },2000);
    });
    ...
    $(document).on('click','.cg_go_to_link',function (e) {
        e.preventDefault();
        //cgViewOptionCheck(this,e);
        var $cgGoToTarget = $('.cg_go_to_target'+'[data-cg-go-to-target="'+$(this).attr('data-cg-go-to-link')+'"]');
