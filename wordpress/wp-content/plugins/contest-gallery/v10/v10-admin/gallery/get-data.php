...
$fieldsToSelectString = "id, rowid, Timestamp, NamePic, ImgType, CountC, CountR, CountS, Rating, GalleryID, Active, Informed, WpUpload,
             Width, Height, WpUserId, rSource, rThumb, addCountS, addCountR1, addCountR2, addCountR3, addCountR4, addCountR5, Category, Exif,
             IP, CountR1, CountR2, CountR3, CountR4, CountR5, Version, CheckSet, CookieId, Winner";
...
if($isAjaxCall){
    ...
    if($isNewGallery){
        ...
        if($search==='' && !($order=='rating_desc_average' || $order=='rating_asc_average' || $order=='rating_desc_average_with_manip' || $order=='rating_asc_average_with_manip') && strpos($order, '_for_id_') === false){
...
if($search===''){
    ...
    $rows = $wpdb->get_var( "SELECT COUNT(*) AS NumberOfRows FROM $tablename WHERE GalleryID = '$GalleryID' $selectWinnersOnly$selectActiveOnly$selectInactiveOnly" );
...
/*
    ...
    foreach($allWpPosts as $wpPostRow){
     ...
     /*echo "<pre>";
       print_r($allWpPostsByWpUploadIdArray);
       echo "</pre>";*/
