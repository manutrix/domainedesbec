...
WHERE GalleryID IN ($oldGalleryID)";
            ...
            if(!empty($oldAndNextGalleryIdsCategories)){
                ...
                $query = "UPDATE $tablename_ip SET Category = CASE Category $whenThenString ELSE Category END WHERE CategoriesOn = 0 AND GalleryID IN ($nextGalleryID)";
                $wpdb->query($query);

                //Same for categories
                // have to be done two times, CategoriesOn = 0 and CategoriesOn = 1
                $query = "UPDATE $tablename_ip SET Category = CASE Category $whenThenString ELSE Category END WHERE CategoriesOn = 1 AND GalleryID IN ($nextGalleryID)";
                $wpdb->query($query);
