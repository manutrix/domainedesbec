...
function tg_testimonial_slider_func($atts, $content) {
	...
	if(!empty($testimonial_arr) && is_array($testimonial_arr))
	{
		...
		foreach($testimonial_arr as $key => $testimonial)
		{
			...
			$return_html.= '<li>';
			$return_html.= '<div class="testimonial_slider_wrapper testimonial_speech_bubble">';
...
function tg_testimonial_list_func($atts, $content) {
	extract(shortcode_atts(array(
		'size' => 'one',
		'items' => 3,
		'fontcolor' => '',
	...
	if(!empty($testimonial_arr) && is_array($testimonial_arr))
	{
		...
		$return_html.= '>';
		$return_html.= '<div class="flexslider" data-height="750">';
		$return_html.= '<ul class="slides list">';

		foreach($testimonial_arr as $key => $testimonial)
		{
			$testimonial_ID = $testimonial->ID;

			$chalet = wp_get_post_terms($testimonial_ID, 'testimonialcats');
			$chalet_name = $chalet[0]->name;
			...
			$return_html.= '<li style="display: block">';
			$return_html.= '<div class="testimonial_slider_wrapper testimonial_speech_bubble">';
			...
			if(!empty($testimonial_name))
			{
				...
				$return_html.= '>'.$testimonial_name.', ' . $chalet_name . '</h6>';
...
add_shortcode('tg_testimonial_list', 'tg_testimonial_list_func');
